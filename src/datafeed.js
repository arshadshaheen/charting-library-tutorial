...
const configurationData = {
	...
	supported_resolutions: ["1", "15", "240",'1D', '1W', '1M'],
...
export default {
	...
	resolveSymbol: async (
		...
		minmov: 1,
		pricescale: 100,
		has_intraday: true,                 // Enable intraday data
		has_no_volume: true,
		has_weekly_and_monthly: false,
		supported_resolutions: configurationData.supported_resolutions,
		volume_precision: 2,
		data_status: 'streaming',
		force_session_rebuild: false,       // Don't force session rebuild
		has_empty_bars: false,              // Don't show empty bars
		};
	...
	getBars: async (symbolInfo, resolution, periodParams, onHistoryCallback, onErrorCallback) => {
		...
		// Use different API endpoints based on resolution
		let apiEndpoint = 'data/histoday'; // Default for daily
		if (resolution === '1' || resolution === '15' || resolution === '240') {
			apiEndpoint = 'data/histominute'; // For intraday timeframes
		}
		...
		// Add timeframe parameter for intraday data
		if (apiEndpoint === 'data/histominute') {
			urlParameters.aggregate = resolution;
		}
		...
		const query = Object.keys(urlParameters)
			...
			.join('&');
		try {
			const data = await makeApiRequest(`${apiEndpoint}?${query}`);
